...
* 모바일 환경에 최적화 되어있습니다. (모바일에서 사용하려면 쉐이더를 가볍게 만들어야 합니다.)
------
...
6월 22일
* 데모씬 잔디가 예쁘지 않아서 쉐이더 새로 추가함
  
7월 2일  
* 유니티 6에서 Draw Mesh Instanced Indirect쉐이더가 작동 안하면 문제 해결
* 잔디 심을때 바운드 추가(만약 이전에 심었다면 업데이트 하면 다시 심으셔야 합니다.)
* 프러스텀 컬링 좀더 정확하게 작동합니다.
* GrassDataList에 압축 버튼 추가 (바운드도 같이 저장되면서 용량이 커졌기 때문에 소숫점 버리는 압축버튼추가했습니다.)
  
7월 3일
* 씬 뷰 에서 느려지던 현상을 해결하였습니다.  

8월 13일
* Ctrl키를 누르고 삭제할때 모든 잔디가 지워지는 방식에서 선택한 잔디만 지워지도록 변경했습니다.  
* 그림자 관련 옵션이 추가되었습니다.

##### 주의사항
~~빌드 세팅이 Windows, Max, Linux(StandAlone) 모드에서는 씬뷰에서 드로우콜이 누적되는 문제가 있어서 Max Cull Distance를 낮춰서 작업해주세요. 게임 플레이에는 문제 없습니다.  
안드로이드 모드 일때는 정상적으로 작동합니다.~~ (해결됨)  
...
Shadow URP Distance : URP Asset에서 그림자 거리만큼 그림자 컬링을 결정합니다.  
Shadow Diatance : Shadow URP Distance를 비활성화 할 경우 이 옵션이 보이게 되고, 입력한 수치만큼 그림자 거리를 조절할수 있지만 URP Asset 설정된 거리까지만 표시됩니다.  
Shadow Frustum Culling : 만약 나무와 같이 커다란것을 심었을 경우 다른곳을 바라봤을때 그림자도 같이 컬링되는 문제를 방지합니다. 대신 그림자의 프러스텀 컬링이 제외 되었기 때문에 GPU사용량이 증가할수 있습니다.  
![easyme](/img/014.png) 
잔디처럼 작으면 컬링을 하는것이 좋지만 나무처럼 커다랗다면 그림자 문제가 발생할수 있습니다.
