...
public class HY_GrassDetailTool : EditorWindow
{
    ...
    private void BakeTabTerrain()
    {
        ...
        overlapPrevention = (OverlapPreventionMode)EditorGUILayout.EnumPopup(new GUIContent("Overlap Prevention", "같은 위치에 같은 오브젝트가 겹치지 않도록 조절합니다."), overlapPrevention);
        switch (overlapPrevention)
        {
            case OverlapPreventionMode.Single:
                overlapPreventionSingle = EditorGUILayout.Slider("Scale", overlapPreventionSingle, 0, 2);
        ...
        if (terrainTargets.Count > 0)
        {
            ...
            EditorGUILayout.HelpBox($"예상 잔디 디테일 수: {formatted} 개", MessageType.Info);

            if (estimated > 200_000)
            {
                EditorGUILayout.HelpBox("예상 잔디 디테일 수가 너무 많습니다. 성능에 영향이 있을 수 있습니다.", MessageType.Warning);
            }
        }
    }
    ...
    private void TerrainGrassDetailBake()
    {
        ...
        foreach (var terrain in terrainTargets)
        {
            ...
            for (int i = 0; i < tData.detailPrototypes.Length; i++)
            {
                ...
                List<Vector2Int> paintedPositions = new();
                ...
                for (int y = 0; y < detailHeight; y++)
                {
                    for (int x = 0; x < detailWidth; x++)
                    {
                        if (detailLayer[y, x] > 0)
                            paintedPositions.Add(new Vector2Int(x, y));
                    }
                }

                if (paintedPositions.Count == 0) continue;
                ...
                float prefabArea = Mathf.Max(visualSize * visualSize, 0.01f); // visualSize는 프리팹의 XZ 최대값
                float baseCount = cellArea / prefabArea;

                float density = Mathf.Clamp(proto.density, 0.01f, 3f);
                int perCellCount = Mathf.Clamp(Mathf.RoundToInt(baseCount * Mathf.Pow(density, 2f)), 1, 255);

                int saveBatchSize = 20000;
                int currentBatchCount = 0;

                foreach (var posXY in paintedPositions)
                {
                    for (int k = 0; k < perCellCount; k++)
                    {
                        float normalizedX = (posXY.x + Random.value) / detailWidth;
                        float normalizedZ = (posXY.y + Random.value) / detailHeight;
                        ...
                        Vector3 scale = new(width, height, width);

                        float overlapRadius = 0f;

                        if (overlapPrevention == OverlapPreventionMode.Single)
                        {
                            overlapRadius = overlapPreventionSingle * scale.x;
                        }
                        else if (overlapPrevention == OverlapPreventionMode.Multi)
                        {
                            overlapRadius = new Vector3(
                                overlapPreventionMulti.x * scale.x,
                                overlapPreventionMulti.y * scale.y,
                                overlapPreventionMulti.z * scale.z
                            ).magnitude;
                        }

                        if (IsOverlapping(pos, occupiedPositions, overlapRadius))
                            continue;
                        ...
                        {
                            currentBatchCount = 0;
                            Debug.Log($"{totalConverted}개 저장 중간처리...");
        ...
        Debug.Log($"변환 완료! 총 {totalConverted}개의 디테일이 배치되었습니다.");
    }
    ...
    private int EstimateTotalGrassCount()
    {
        ...
        foreach (var terrain in terrainTargets)
        {
            ...
            float cellSize = tData.size.x / tData.detailResolution;
            int detailWidth = tData.detailWidth;
            int detailHeight = tData.detailHeight;
            ...
            {
                var proto = tData.detailPrototypes[i];
                if (proto.prototype == null && proto.prototypeTexture == null) continue;
                ...
                // 프리팹 사이즈
                float prefabSize = proto.prototype == null ? 1f : GetPrefabVisualSize(proto.prototype);
                float scale = Mathf.Max((proto.minWidth + proto.maxWidth) * 0.5f, 0.01f);
                float visualSize = prefabSize * scale;

                float cellArea = cellSize * cellSize;
                float prefabArea = Mathf.Max(visualSize * visualSize, 0.01f);

                float baseCount = cellArea / prefabArea;
                float density = Mathf.Clamp(proto.density, 0.01f, 3f);
                int perCellCount = Mathf.Clamp(Mathf.RoundToInt(baseCount * Mathf.Pow(density, 2f)), 1, 255);

                total += paintedCells * perCellCount;
            }
        }
    ...
    private List<GameObject> GetOnlyDirectChildPrefabs(GameObject parent)
    {
        List<GameObject> prefabs = new List<GameObject>();
        ...
        foreach (Transform child in parent.transform)
        {
            ...
            if (childPrefab != null) // 프리팹인지 확인
            {
                prefabs.Add(child.gameObject); // 한 번만 추가
            }
            else
            {
                // 프리팹이 아니면, 그 아래 자식들 중 프리팹을 찾음
                prefabs.AddRange(GetOnlyDirectChildPrefabs(child.gameObject));
            }
        }
